...
def save_logs(train_log, valid_log):
    np.save("unet-32-epoch-40-focal/train_log.npy", train_log)
    np.save("unet-32-epoch-40-focal/valid_log.npy", valid_log)


DEVICE = "cuda" if torch.cuda.is_available() else "cpu"


criterion = smp.losses.FocalLoss(mode='multiclass', gamma=2.0)  # The SMP library also contains various loss functions
criterion.name = 'FocalLoss'
...
for i in range(0, EPOCHS):
    ...
    if (min_score > valid_mean):
        ...
        torch.save(model.state_dict(), 'unet-32-epoch-40-focal/best_model_dict.pth')
        print("Model saved!")
    if i == EPOCHS / 2:
