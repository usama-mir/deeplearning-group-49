...
valid_loader = DataLoader(validation_dataset, batch_size=1, shuffle=False, num_workers=2, drop_last=True)

# We define the model:
model = smp.DeepLabV3(
    encoder_name='resnet34',  # We use the ResNeSt 200 backbone
...
def save_logs(train_log, valid_log):
    np.save("deeplab-32-epoch-40-dice/train_log.npy", train_log)
    np.save("deeplab-32-epoch-40-dice/valid_log.npy", valid_log)
...
for i in range(0, EPOCHS):
    ...
    if (min_score > valid_mean):
        ...
        torch.save(model.state_dict(), "deeplab-32-epoch-40-dice/best_model_dict.pth")
        print("Model saved!")
    if i == EPOCHS / 2:
